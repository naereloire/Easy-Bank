...
const Home = () => {
	...
	const getBalance = (titularId) => {
		setShow(true);
		easyBankApi
			.get(`account/titular_id/${titularId}`)
			.then((resp) => {
	...
	const TransferAccount = () => {
		easyBankApi
			.get(`titular/cpf/${transferCPF}`)
			.then((response) => {
				if (response.data.id) {
					const destinyTitularId = response.data.id;
	...
	useEffect(() => {
		firebase.auth().onAuthStateChanged((user) => {
			...
			easyBankApi
				.get(`account/titular_id/${user.photoURL}`)
				.then((resp) => {
			...
			<Modal show={showTransfer} id='form-transfer'>
				<DivCenter>
					<StyleInput
						...
						placeholder='cpf'
						type='text'></StyleInput>
					<StyleInput
						onBlur={(event) => {
							setTransferValue(event.target.value);
						}}
						placeholder='valor'
						type='number'></StyleInput>
					<DivButtons>
